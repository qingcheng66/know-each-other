...
0. **启动**：读 `.collab/sessions/` + `.collab/locks/` 看谁活着、哪些文件域被锁；写自己的注册文件 `.collab/sessions/<会话名>.md`（会话名由用户指定，如 agent-a / agent-b）
1. **认领**：读 `.collab/board.md` 认领 ⏳ 任务 → 标记 🔄 + 会话名；**board 变更必须跟一次 git commit**（防多会话覆盖）
...
6. **提交纪律**：开工前 `git status` 必须干净（有别人未提交的改动先停下）；push 前必 `git pull --rebase`；一个会话一个端口/环境（如 dev 3000 / 3001）；别同时跑构建（.next / dist / target 会被踩）
7. **结束**：释放自己所有锁 + 删除注册文件 + 带会话名写 state

### 文件域（按项目结构自定义）

示例（前端项目常见形态）：`styles`(全局样式/主题) | `layout`(根布局/外壳) | `content-data`(数据层/内容文件) | `components`(组件目录) | `pages`(页面目录) | `admin`(后台+API) | `infra`(构建/部署/配置) | `collab`(.collab/自身)
...
[HH:MM] agent-a — ⚠️ {影响其他模块的改动说明}
[HH:MM] agent-a — 完成 {任务}，commit {hash}
```
...
认领时把任务从 ⏳ 移到 🔄，标注会话名和时间：
```
- REQ-004 前端登录页面 (agent-a, 14:30)
```

完成时移到 ✅：
```
- REQ-004 前端登录页面 (agent-a, a1b2c3d, 7/28)
```
...
echo "agent-a $(date '+%m-%d %H:%M')" > .collab/locks/components.lock/owner
